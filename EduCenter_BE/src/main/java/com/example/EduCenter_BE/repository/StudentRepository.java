...
public interface StudentRepository extends JpaRepository<Student, Long> {
    ...
    @Query("SELECT s FROM Student AS s WHERE s.isDeleted = false AND (LOWER(s.fullName) LIKE LOWER(CONCAT('%', :keyword, '%')) OR LOWER(s.studentCode) LIKE LOWER(CONCAT('%', :keyword, '%')) OR LOWER(s.email) LIKE LOWER(CONCAT('%', :keyword, '%')) OR LOWER(s.phone) LIKE LOWER(CONCAT('%', :keyword, '%'))) ORDER BY s.createdAt DESC")
